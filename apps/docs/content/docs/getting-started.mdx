...
`defineContainer<Intent>()` is curried: the optional first call fixes the
effect-intent union (see [Effects](/docs/effects)); the second call infers
state, actions, and derivations from the body.
